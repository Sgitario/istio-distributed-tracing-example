...
For the integration tests to pass, you need to disable the security of Jaeger UI first. By default, when installing the Red Hat OpenShift distributed tracing platform operator, it will install a Jaeger instance with the security enabled:
...
To disable, you need to update the Jaeger instance with:
...
And update it. The operator will automatically reload the Jaeger instance with the new configuration.

Next, we can run the integration tests as follows:
```bash
